"""
Enhanced SOL-USD-PERP Data Resolver

This module specifically addresses the SOL-USD-PERP data acquisition issues
by implementing specialized handling for this token.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("sol_data_resolver.log")
    ]
)
logger = logging.getLogger(__name__)

class SOLDataResolver:
    """
    Specialized data resolver for SOL-USD-PERP.
    """
    
    def __init__(self, base_dir: str = "real_market_data"):
        """
        Initialize the SOL data resolver.
        ...
        logger.info("SOL Data Resolver initialized")
        
    async def fetch_sol_data(self, interval: str, days: int = 30) -> Tuple[pd.DataFrame, Dict]:
        """
        Fetch SOL-USD-PERP data from multiple sources with fallback.
        ...
        """
        symbol = "SOL-USD-PERP"
        logger.info(f"Fetching SOL-USD-PERP data for interval {interval}, days {days}")
        ...
        try:
            ...
            df_hyperliquid = await self.fetch_from_hyperliquid_enhanced(interval, days)
        ...
        # Try to fetch from Kraken (added as new source)
        try:
            logger.info(f"Attempting to fetch data from Kraken for {symbol}, interval {interval}")
            df_kraken = await self.fetch_from_kraken(interval, days)
        ...
        # Try to fetch from Bybit (added as new source)
        try:
            logger.info(f"Attempting to fetch data from Bybit for {symbol}, interval {interval}")
            df_bybit = await self.fetch_from_bybit(interval, days)
            
            if df_bybit is not None and not df_bybit.empty:
                logger.info(f"Successfully fetched data from Bybit for {symbol}, interval {interval}")
                alternative_sources.append("bybit")
                
                # Save raw data
                df_bybit.to_csv(f"{self.base_dir}/raw/{symbol}_{interval}_bybit_klines.csv", index=False)
                
                # Calculate technical indicators
                df_processed = self.calculate_technical_indicators(df_bybit)
            ...
            error_msg = f"Failed to fetch data from Bybit for {symbol}, interval {interval}: {str(e)}"
            logger.warning(error_msg)
            # Log detailed error information
            logger.error(f"Bybit API error details: {type(e).__name__}, {str(e)}")
        
        # Try to fetch from Binance
        try:
            logger.info(f"Attempting to fetch data from Binance for {symbol}, interval {interval}")
            df_binance = await self.fetch_from_binance(interval, days)
        ...
        try:
            ...
            df_coingecko = await self.fetch_from_coingecko(interval, days)
        ...
        # Try to fetch from FTX (as last resort)
        try:
            logger.info(f"Attempting to fetch data from FTX for {symbol}, interval {interval}")
            df_ftx = await self.fetch_from_ftx(interval, days)
            
            if df_ftx is not None and not df_ftx.empty:
                logger.info(f"Successfully fetched data from FTX for {symbol}, interval {interval}")
                alternative_sources.append("ftx")
                
                # Save raw data
                df_ftx.to_csv(f"{self.base_dir}/raw/{symbol}_{interval}_ftx_klines.csv", index=False)
                
                # Calculate technical indicators
                df_processed = self.calculate_technical_indicators(df_ftx)
            ...
            error_msg = f"Failed to fetch data from FTX for {symbol}, interval {interval}: {str(e)}"
            logger.warning(error_msg)
            # Log detailed error information
            logger.error(f"FTX API error details: {type(e).__name__}, {str(e)}")
        ...
        try:
            ...
            df_hybrid = await self.create_hybrid_dataset(interval, days)
        ...
        df_synthetic = self.generate_synthetic_data(interval, days)
    ...
    async def fetch_from_hyperliquid_enhanced(self, interval: str, days: int = 30) -> pd.DataFrame:
        """
    ...
    async def fetch_from_binance(self, interval: str, days: int = 30) -> pd.DataFrame:
        """
    ...
    async def fetch_from_coingecko(self, interval: str, days: int = 30) -> pd.DataFrame:
        """
    ...
    async def fetch_from_kraken(self, interval: str, days: int = 30) -> pd.DataFrame:
        """
    ...
    async def fetch_from_bybit(self, interval: str, days: int = 30) -> pd.DataFrame:
        """
        ...
        mapped_symbol = self.symbol_mappings[symbol]["bybit"]
        ...
        mapped_interval = self.interval_mappings[interval]["bybit"]
        ...
        # Construct URL
        url = f"{self.bybit_url}/kline/list?symbol={mapped_symbol}&interval={mapped_interval}&from={start_time}&limit=200"
        
        # Log the request details
        logger.info(f"Bybit API request: URL={url}")
        ...
        try:
            ...
            logger.info(f"Bybit API response headers: {response.headers}")
            ...
            try:
                data = response.json()
                logger.info(f"Bybit API response data structure: {list(data.keys()) if isinstance(data, dict) else 'Not a dict'}")
            except json.JSONDecodeError:
                logger.error(f"Bybit API response is not valid JSON: {response.text[:200]}...")
                return None
            
            # Check if data is valid
            if not data or "result" not in data or not data["result"]:
                logger.warning(f"No data returned from Bybit for {symbol}, interval {interval}")
                return None
            ...
            klines = []
            for kline in result:
                timestamp = datetime.fromtimestamp(kline["open_time"])
                open_price = float(kline["open"])
                high_price = float(kline["high"])
                low_price = float(kline["low"])
                close_price = float(kline["close"])
            ...
            logger.error(f"Bybit API request error: {type(e).__name__}, {str(e)}")
            
            # If we have a response, log its content
            if 'response' in locals():
                logger.error(f"Bybit API error response: {response.text[:500]}...")
                
            return None
        except Exception as e:
            logger.error(f"Unexpected error fetching data from Bybit: {type(e).__name__}, {str(e)}")
            return None
            
    async def fetch_from_ftx(self, interval: str, days: int = 30) -> pd.DataFrame:
        """
        Fetch data from FTX.
        ...
        mapped_symbol = self.symbol_mappings[symbol]["ftx"]
        ...
        mapped_interval = self.interval_mappings[interval]["ftx"]
        ...
        # Construct URL
        url = f"{self.ftx_url}/markets/{mapped_symbol}/candles?resolution={mapped_interval}&start_time={start_time}&end_time={end_time}"
        
        # Log the request details
        logger.info(f"FTX API request: URL={url}")
        ...
        try:
            ...
            # Log response status and headers
            logger.info(f"FTX API response status: {response.status_code}")
            logger.info(f"FTX API response headers: {response.headers}")
            ...
            try:
                data = response.json()
                logger.info(f"FTX API response data structure: {list(data.keys()) if isinstance(data, dict) else 'Not a dict'}")
            except json.JSONDecodeError:
                logger.error(f"FTX API response is not valid JSON: {response.text[:200]}...")
                return None
            
            # Check if data is valid
            if not data or "result" not in data or not data["result"]:
                logger.warning(f"No data returned from FTX for {symbol}, interval {interval}")
                return None
            ...
            logger.error(f"FTX API request error: {type(e).__name__}, {str(e)}")
            
            # If we have a response, log its content
            if 'response' in locals():
                logger.error(f"FTX API error response: {response.text[:500]}...")
                
            return None
        except Exception as e:
            logger.error(f"Unexpected error fetching data from FTX: {type(e).__name__}, {str(e)}")
            return None
            
    async def create_hybrid_dataset(self, interval: str, days: int = 30) -> pd.DataFrame:
        """
        ...
        bybit_path = f"{self.base_dir}/raw/{symbol}_{interval}_bybit_klines.csv"
        if os.path.exists(bybit_path):
            try:
                df_bybit = pd.read_csv(bybit_path)
                df_bybit["timestamp"] = pd.to_datetime(df_bybit["timestamp"])
                df_bybit["source"] = "bybit"
                partial_dfs.append(df_bybit)
                logger.info(f"Added Bybit data to hybrid dataset: {df_bybit.shape[0]} rows")
            except Exception as e:
                logger.error(f"Error reading Bybit data: {e}")
                
        # Check for FTX data
        ftx_path = f"{self.base_dir}/raw/{symbol}_{interval}_ftx_klines.csv"
        if os.path.exists(ftx_path):
            try:
                df_ftx = pd.read_csv(ftx_path)
                df_ftx["timestamp"] = pd.to_datetime(df_ftx["timestamp"])
                df_ftx["source"] = "ftx"
                partial_dfs.append(df_ftx)
                logger.info(f"Added FTX data to hybrid dataset: {df_ftx.shape[0]} rows")
            except Exception as e:
                logger.error(f"Error reading FTX data: {e}")
    ...
    def generate_synthetic_data(self, interval: str, days: int = 30) -> pd.DataFrame:
        """
    ...
    def calculate_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Add funding rate (synthetic for SOL)
        np.random.seed(42)  # For reproducibility
...
async def main():
    """
    Main function to test the SOL data resolver.
    """
    # Parse command line arguments
    import argparse
    parser = argparse.ArgumentParser(description="SOL Data Resolver")
    ...
    for interval in intervals:
        logger.info(f"Fetching data for SOL-USD-PERP, interval {interval}")
        df, data_quality = await resolver.fetch_sol_data(interval, args.days)
        
        logger.info(f"Data quality for SOL-USD-PERP, interval {interval}: {data_quality}")
