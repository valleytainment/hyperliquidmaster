...
class SecurityManager:
    ...
    def store_private_key(self, private_key: str, method: str = "auto") -> bool:
        """
        Store private key using the specified method
        
        Args:
            private_key: The private key to store
            method: Method to use ("auto", "encrypted_file", "keyring", "env")
            
        Returns:
            bool: True if successful
        """
        if not private_key:
            logger.error("Cannot store empty private key")
            return False
        ...
        if method == "auto":
            ...
            methods = ["keyring", "encrypted_file"]
        else:
            methods = [method]
        ...
        for m in methods:
            try:
                if m == "keyring":
                    ...
                    success = self.store_in_keyring("private_key", private_key)
                    if success:
                        logger.info("Private key stored in system keyring")
                        return True
                
                elif m == "encrypted_file":
                    # Store in encrypted file
                    # Use a default password for automated storage
                    default_password = "hyperliquid_default_password"
                    success = self.encrypt_private_key(private_key, default_password)
                    if success:
                        logger.info("Private key stored in encrypted file")
                        return True
                
                elif m == "env":
                    # Just log a message - we can't set env vars programmatically
                    logger.info("To store in environment, set HYPERLIQUID_PRIVATE_KEY manually")
                    return False
        ...
        logger.error("Failed to store private key using any method")
        return False
    ...
    def decrypt_private_key(self, password: str = None) -> Optional[str]:
        ...
        try:
            ...
            if not password:
                # Try default password first for automated retrieval
                password = "hyperliquid_default_password"
            ...
            try:
                ...
                # If default password fails and no password was provided, prompt
                if password == "hyperliquid_default_password" and not password:
    ...
    def clear_all_credentials(self) -> bool:
        ...
        try:
            ...
            if self.encrypted_file.exists():
                if not self.secure_delete_file(self.encrypted_file):
                    success = False
    ...
    def __str__(self) -> str:
        ...
        has_encrypted_file = self.encrypted_file.exists()
        has_keyring = self.get_from_keyring("private_key") is not None
        
        return f"SecurityManager(encrypted_file={has_encrypted_file}, keyring={has_keyring})"
